...
xs.min? = some x ↔ x ∈ xs ∧ ∀ y ∈ xs, x ≤ y := by
  induction xs generalizing x; simp; clear! xs; rename_i z xs ih
  simp; cases h : xs.min?; aesop; rename_i m; dsimp; simp [ih] at h
...
(∀ n, series (F $ a ·) n = series (F $ a $ σ₁ ·) (f n) + series (F $ a $ σ₂ ·) (g n)) := by
  ...
  · exact λ _ _ => Finset.card_filter_range_le_of_le
  · intro H n
    induction n; simp; rename_i n ih
...
theorem mkSubseq_spec {a p n} (h : Infp a p) : p $ a $ mkSubseq a p n := by
  ...
  rename_i n; apply Nat.find!_spec (p := λ k => p (a (mkSubseq a p n + 1 + k)))
...
theorem monoLt_iff_lt_succ {a} : monoLt a ↔ ∀ n, a n < a (n + 1) := by
  ...
  intro h k n hk
  obtain ⟨n, rfl⟩ := Nat.exists_eq_add_of_lt hk; clear hk
  induction n; simp [h]; rename_i n ih
  rw [←Nat.add_assoc]
  exact ih.trans $ h _
...
(bwSeq a y₁ y₂ n).1 < (bwSeq a y₁ y₂ n).2 := by
  ...
  rename_i n ih; split_ifs <;> apply ih <;> linarith
...
(h : ∀ n, σ n < σ (n + 1)) : n ≤ σ n := by
  induction n; simp; rename_i n ih; rw [Nat.succ_le_iff]; exact lt_of_le_of_lt ih $ h _
...
(List.range (σp a n) |>.filter (0 ≤ a ·) |>.map a).sum = series (ap a) n := by
  ...
  rename_i n ih; simp [series_succ, ←ih]; unfold ap σp
...
(List.range (σn a n) |>.filter (a · < 0) |>.map a).sum = series (an a) n := by
  ...
  rename_i n ih; simp [series_succ, ←ih]; unfold an σn
